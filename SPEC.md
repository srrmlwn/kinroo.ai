...
The landing page (`web/`, served at the root route) is marketing-only: no login, no app functionality, and deliberately styled apart from the rest of the app — a dark, high-contrast palette (`#09090b` background, `#f4f4f5` headings, `#a1a1aa` body, a single blue accent) scoped to this one page's own markup rather than the shared `globals.css`, so `/settings` and the rest of the authenticated app keep their own light/auto theme. A `POST /api/waitlist` route (backed by a standalone `waitlist_signups` table — no `userId`, no auth, not part of the product's own data model) collects pre-launch signups from the hero, replacing the old static "not yet on the Chrome Web Store" line.

It embeds three short looping demos (`web/public/demo/`) behind a tabbed switcher, one per core workflow — a typed request, a flyer/image upload, and the page-detection scan — all of the actual side panel captured from the real built extension (headless Chromium driving the genuine app code against stubbed API responses, not a mockup or an AI-generated video), framed in a browser-chrome-style card.
...
5. The extension stores the session JWT in `chrome.storage.local` and sends it in an `x-kinroo-session` header on every subsequent call — deliberately not the standard `Authorization` header, which Vercel's edge network intercepts for its own deployment-protection checks even on domains meant to be exempt from it (confirmed in production: the header the extension sent was simply absent by the time the route handler ran). The backend verifies it, maps it to a user, and uses that user's stored (encrypted) Google refresh token to call the Calendar API server-side — the extension never sees the Google token directly.
