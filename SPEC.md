...
7. Web settings page (`/settings`) — timezone, default event duration, and calendar ID are editable outside the extension. Since there's no hosted web login, the extension's popup mints a short-lived handoff token that the settings page exchanges for a session cookie (`api/auth/handoff`) rather than the app growing a second OAuth flow.
