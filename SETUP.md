...
4. **Extension**: for a build meant to talk to the deployed backend, set `apiBase` in your local (gitignored) `extension/config.json` to `https://kinroo.ai`, then rebuild (`extension/config.json` is copied into `extension/dist/` at build time, not read from disk at runtime) and reload the extension. `host_permissions` in `extension/manifest.json` already includes both `https://kinroo.ai/*` and `https://www.kinroo.ai/*` alongside the localhost dev entry — both are needed because the apex domain redirects to `www`, and a background-script fetch that gets redirected to a host outside `host_permissions` loses its CORS exemption and gets blocked by the browser.
...
6. **Email ingest** (if using it): now that there's a public HTTPS URL, come back to SETUP.md §10 step 3 and point SendGrid's Inbound Parse destination URL at `https://kinroo.ai/api/email/inbound?secret=<EMAIL_INGEST_WEBHOOK_SECRET>`.
